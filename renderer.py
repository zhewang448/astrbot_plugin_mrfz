...
class VoiceRenderer:
    ...
    def _read_font_bytes(self, candidate: Path) -> Optional[bytes]:
        """Cache and return font bytes so Pillow never keeps the file handle.

        On Windows ``ImageFont.truetype(path)`` holds the file open for the
        whole lifetime of the font object, which blocks AstrBot from deleting
        the plugin folder (e.g. the bundled ``.otf``) on update/uninstall.
        Loading bundled fonts from an in-memory buffer avoids that lock.
        """
        key = str(candidate)
    ...
    def _load_font(self, size: int, *, bold: bool = False, mono: bool = False):
        ...
        for candidate in self._font_candidates(bold=bold, mono=mono):
            try:
                ...
                # Fonts bundled inside the plugin folder are loaded from an
                # in-memory copy so the plugin never locks the file. System
                # fonts stay path-based since AstrBot never deletes them.
