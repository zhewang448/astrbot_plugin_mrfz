...
class MyPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        try:
            ...
            self.language_list = [
                ...
                "kr",
                "it",
            ]  # 1:方言, 2:汉语, 3:日语, 4:英语, 5:韩语, 6:意大利语
            self.default_language_rank = self.config.get(
                "default_language_rank", "123456"
            )
            self.auto_download_language = self.config.get(
                "auto_download_language", "123"
            )
            self.language_dic = {
                "中文-普通话": "cn",
                "英语": "us",
                ...
                if lang_dir.name == "skin":
                    ...
                    for skin_lang_dir in lang_dir.iterdir():
                        if not skin_lang_dir.is_dir():
                            ...
                            skin_lang_dir.is_dir()
                            and not len(os.listdir(skin_lang_dir)) == 0
                        ):
    ...
    def _get_voice_path(
        ...
        """
        logger.info(
            f"尝试获取语音路径: 角色={character}, 语音={voice_name}, 语言={language}"
        )
        ...
        base_path = (
            char_dir / language
            if not is_skin_character
            else char_dir / "skin" / language
        )
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            avatar_url, headers=self.DEFAULT_HEADERS
                        ) as response:
                            if response.status == 200:
                                ...
                                logger.warning(
                                    f"{character}头像下载失败，状态码：{response.status}"
                                )
                except Exception as e:
                    logger.error(
                        f"{e}[ensure_assets] {character}头像下载出错: {avatar_url}"
                    )
            else:
    ...
    async def fetch_operator_avatar(self, op_name: str) -> Tuple[str, str]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                file_page_url, headers=self.DEFAULT_HEADERS, timeout=10
            ) as response:
                if response.status == 200:
                    ...
                    image_url_match = re.search(
                        r'<meta property="og:image" content="([^"]+)"', file_page_html
                    )
                    if image_url_match:
    ...
    async def download_voice(
        self, character: str, voice_url: str, language: str, description: str
    ) -> Tuple[bool, str]:
        """下载单个语音文件
        Args:
            character (str): 角色名
        ...
        try:
            ...
            while retry_count < MAX_RETRIES:
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            voice_url, headers=self.DEFAULT_HEADERS
                        ) as response:
                            if response.status != 200:
                                retry_count += 1
    ...
    async def get_character_id(self, character: str) -> Optional[Dict]:
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://prts.wiki/w/{character}/语音记录"
                ) as resp:
                    ...
                    for lang_path in voice_data.split(","):
                        ...
                        if "char_" in path and "语音key" not in result:
                            result["语音key"] = re.search(r"char_\d+_\w+", path).group(
                                0
                            )
    ...
    async def fetch_character_voices(self, character: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            for lang, key in char_id_list.items():
                if (
                    lang == "语音key"
                    or self.language_dic[self.language_dic[lang.split("(")[0].strip()]]
                    not in self.auto_download_language
                ):  # 跳过元数据字段和非自动下载语言
                    continue
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.head(
                                ...
                                if response.status == 200:
                                    ...
                                    success, message = await self.download_voice(
                                        character, voice_url, lang, description
                                    )
                                    if success:
                                        total_voices += 1
                                        ...
                                        failed_voices += 1
                                        logger.warning(
                                            f"下载失败 ({lang}语音{file_idx}): {message}"
                                        )
                                else:
            ...
            # 在下载完成后立即扫描更新索引
            if total_voices > 0:
                logger.info("下载完成，开始扫描语音文件更新索引...")
                self.scan_voice_files()
                logger.info("语音索引更新完成")

            # 构建返回消息
            result_msg = []
            if total_voices > 0 or failed_voices or True:
                result_msg.append(
                    f"成功语音: {total_voices}个"
                    + (f"(失败{failed_voices}个)" if failed_voices > 0 else "")
                )
        ...
        for rank in self.default_language_rank:
            try:
                ...
                if 0 <= lang_index < len(self.language_list):
                    ...
                    # 检查该语言下是否有语音文件
                    if not is_skin_character and (
                        potential_lang in self.voice_index[character]
                    ):
                        lang = potential_lang
                        break
                    elif (
                        is_skin_character
                        and potential_lang
                        in self.voice_index[character.replace("皮肤", "")]
                    ):
                        lang = potential_lang
                        break
                    else:
    ...
    async def mrfz_handler(
        ...
        try:
            ...
            self.scan_voice_files()
            logger.info(
                f"开始处理mrfz命令: 角色={character}, 语音={voice_name}, 语言={language}"
            )
                ...
                if language:
                    ...
                    if language.lower() in self.language_list:
                        ...
                        yield event.plain_result(
                            "语言参数错误,请使用 jp(日语)、cn(中文)、fy(方言)、us(英语)、kr(韩语)、it(意语)"
                        )
                        return
                else:
                    # 根据配置选择语言优先级
            ...
            if lang == "nodownload":
                if not self.auto_download:
                    yield event.plain_result(
                        f"未找到角色 {character} 的{language}语音文件。自动下载已禁用，请使用 /mrfz_fetch 手动获取。"
                    )
                    return

                logger.info(f"未找到角色 {character} 的语音文件，尝试自动下载")
                yield event.plain_result(
                    f"未找到角色 {character} 的语音文件，正在自动获取..."
                )
    ...
    async def mrfz_list_handler(self, event: AstrMessageEvent):
        ...
        try:
            if not self.voice_index:
                yield event.plain_result(
                    "当前没有缓存的语音文件，请先使用 mrfz_fetch 命令获取语音。"
                )
                return
            ...
            for char in self.voice_index:
                ...
                # 如果角色名以"皮肤"结尾，放入皮肤列表
                if char.endswith("皮肤"):
                    skin_character = {"name": char, "languages": [], "avatar_url": ""}
                    ...
                    for lang_code in ["jp", "cn", "fy", "us", "kr", "it"]:
                        if lang_code in self.voice_index[char]:
                            ...
                            skin_character["languages"].append(
                                {"code": lang_code, "display": display_name}
                            )

                    # 获取基础角色名，用于头像
                    base_name = char.replace("皮肤", "")
                    skin_character["avatar_url"] = (
                        f"file://{self.data_dir / 'assets' / base_name}.png".replace(
                            "\\", "/"
                        )
                    )
            ...
            logo_url = os.path.join(
                "file://", str(Path(self.plugin_dir) / "assets" / "logo.png")
            ).replace("\\", "/")
            caution_line_url = os.path.join(
                "file://", str(Path(self.plugin_dir) / "assets" / "caution_line.png")
            ).replace("\\", "/")
            ...
            try:
                ...
                if skin_operators:
                    skin_ops_height = (
                        len(skin_operators) * line_height + 60
                    )  # 每个皮肤角色40像素高度 + 标题和边距
                ...
                logger.info(
                    f"计算图片高度: 标题={title_height}, 语音类型={voice_types_height}, 干员列表={ops_height}, 皮肤列表={skin_ops_height}, 底部={footer_height}, 总计={total_height}"
                )
                ...
                # 绘制语音类型部分
                draw.text(
                    (padding, y), "语音类型总览", fill=(255, 204, 0), font=section_font
                )
                y += 40
                ...
                draw.rectangle(
                    [(padding, y), (img_width - padding, y + voice_bg_height)],
                    fill=(40, 42, 46),
                )
                draw.line(
                    [(padding, y), (img_width - padding, y)],
                    fill=(255, 204, 0),
                    width=3,
                )
                ...
                draw_avatar.text(
                    (16, 16), "?", fill=(255, 204, 0), font=normal_font, anchor="mm"
                )
                ...
                if operators:
                    ...
                    draw.rectangle(
                        [(padding, y), (img_width - padding, y + ops_bg_height)],
                        fill=(40, 42, 46),
                    )
                    ...
                    for i, op in enumerate(operators):
                        ...
                        if i % 2 == 0:
                            draw.rectangle(
                                [
                                    (padding + 2, text_y - line_height // 2 + 2),
                                    (
                                        img_width - padding - 2,
                                        text_y + line_height // 2 - 2,
                                    ),
                                ],
                                fill=(45, 48, 56),
                            )
                        ...
                        try:
                            ...
                            if op["name"] in avatar_cache:
                                ...
                                local_avatar_path = (
                                    self.assets_dir / f"{op['name']}.png"
                                )
                                if local_avatar_path.exists():
                                    try:
                                        avatar_img = PILImage.open(
                                        ...
                                        logger.warning(
                                            f"加载本地头像失败: {op['name']} - {str(e)}"
                                        )
                                else:
                                    # 本地没有，从网络获取
                                    # 获取头像URL
                                    avatar_url = await self.fetch_operator_avatar(
                                        op["name"]
                                    )
                ...
                if skin_operators:
                    draw.text(
                        (padding, y), "皮肤语音", fill=(255, 204, 0), font=section_font
                    )
                    y += 40
                    ...
                    draw.rectangle(
                        [(padding, y), (img_width - padding, y + skin_ops_bg_height)],
                        fill=(40, 42, 46),
                    )
                            ...
                            if base_name in avatar_cache:
                                ...
                                if local_avatar_path.exists():
                                    try:
                                        ...
                                        logger.warning(
                                            f"加载皮肤角色头像失败: {base_name} - {str(e)}"
                                        )
                        except Exception as e:
                            logger.warning(
                                f"获取皮肤角色头像URL失败: {op['name']} - {str(e)}"
                            )
                ...
                powered_by_y = y + 45
                draw.text(
                    (img_width // 2, powered_by_y),
                    "Powered by astrbot_plugin_mrfz",
                    fill=(160, 160, 160),
                    font=small_font,
                    anchor="mm",
                )  # 调亮灰色以增加可见度
                ...
                for op in operators:
                    ...
                    for lang in op["languages"]:
                        ...
                        if (
                            lang_code != "skin"
                            and op["name"] in self.voice_index
                            and lang_code in self.voice_index[op["name"]]
                        ):
                            voice_counts[lang_code] = len(
                                self.voice_index[op["name"]][lang_code]
                            )
                    ...
                    for lang in op["languages"]:
                        ...
                        if lang_code in voice_counts:
                            lang_info.append(
                                f"{lang['display']}({voice_counts[lang_code]})"
                            )
                        else:
    ...
    @filter.command(
        "mrfz_fetch",
        alias={"明日方舟下载语音", "方舟下载语音", "方舟语音下载", "明日方舟语音下载"},
    )
    ...
    async def _fetch_all_avatar_mappings(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"仍有{not_found_count}个干员未找到头像"
            ) if not_found_count > 0 else None
